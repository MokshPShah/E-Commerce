...
export default async function DashboardPage() {
    ...
    // Protect the route: If no session, send to login
    if (!session) {
        redirect("/login");
    }
    ...
    const orderCount = await Order.countDocuments({ user: (session.user as any).id });
        ...
        <UserDashboardShell>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center">
                    ...
                    <a href="/dashboard/orders" className="text-xs font-bold text-[#ec1313] hover:underline uppercase tracking-widest">View All</a>
                </div>
                ...
                <div className="p-2">
                    {recentOrders.length === 0 ? (
                        ...
                        recentOrders.map((order: any) => (
                            <div key={order._id} className="p-4 hover:bg-slate-50 rounded-2xl transition-all flex items-center justify-between group">
