...
export default function Navbar() {
    ...
    // NEW: Highly optimized E-commerce navigation links
    const navLinks = [
    ...
    return (
        ...
        >
                ...
                <div className="flex items-center justify-between h-20">
                        ...
                        {navLinks.map((link) => (
                            <Link 
                                ...
                                className={`text-sm font-bold uppercase tracking-wider transition-colors relative group ${
                                    link.name === "Shop All" ? "text-[#ec1313]" : "text-slate-700 hover:text-[#ec1313]"
                                }`}
                            >
                                {link.name}
                    ...
                    <div className="flex items-center gap-5 md:gap-6">
                        ...
                        <Link href="/cart" className="text-slate-700 hover:text-[#ec1313] transition-colors relative group">
                            ...
                            <span className="absolute -top-2 -right-2 bg-[#ec1313] text-white text-[10px] font-bold h-[18px] w-[18px] rounded-full flex items-center justify-center ring-2 ring-white">
                                0
                            </span>
                        </Link>
            ...
            <div 
                className={`md:hidden absolute top-[100%] left-0 w-full bg-white border-b border-slate-200 shadow-xl transition-all duration-300 overflow-hidden ${
                ...
                <div className="flex flex-col px-6 py-6 space-y-4 bg-slate-50">
                    {navLinks.map((link) => (
                        <Link 
                            ...
                            className={`text-lg font-black uppercase tracking-widest transition-colors border-b border-slate-200 pb-3 ${
                                link.name === "Shop All" ? "text-[#ec1313]" : "text-slate-900 hover:text-[#ec1313]"
                            }`}
                        >
                            {link.name}
                        </Link>
